...
TEMPLATE_BIG = "/content/drive/MyDrive/IMG_1371_nobg.png"
TEMPLATE_MINI = "/content/drive/MyDrive/IMG_0590.png"
# Threshold constants
...
IMAGE_FOLDER_PATH = Path("/content/drive/MyDrive/Dataset/Good Naans")
FRAME_INTERVAL_SEC = 0.5
USE_GPU = True
DEVICE = torch.device("cuda" if USE_GPU and torch.cuda.is_available() else "cpu")

# Thread-safe, time ordered queue
pq = queue.PriorityQueue()          # (timestamp, ndarray)
...
#     if not cap.isOpened():
#         print("Failed to open video"); pq.put((time.time(), _SENTINEL)); return

#     fps = cap.get(cv2.CAP_PROP_FPS) or 30  # fallback
#     every_n_frames = max(1, int(FRAME_INTERVAL_SEC * fps))
#     frame_idx = 0
...
#                 pq.put((time.time(), frame_rgb))
#             else:
#                 print(f"Warning: Encountered a None frame at index {frame_idx}")

#         frame_idx += 1

#     cap.release()
#     # Tell consumer we're done
#     pq.put((time.time(), _SENTINEL))
...
def reader_thread():
    ...
    for idx, image_path in enumerate(image_files):
        ...
        profile = FrameProfile(idx=idx)              # NEW
        pq.put((profile.t_enqueue, idx, profile, frame))  # timestamp is first
                                                     # keeps PQ ordering
    # at EOF
    pq.put((time.time() - t, float('inf'), None, _SENTINEL))
...
def worker_allocator():
    ...
    while True:
        ts, idx, prof, frame = pq.get() # ts is first by PQ order

        if frame is _SENTINEL:
            print("Shutdown signal received. Shutting down workAllocator...")
            pq.put((time.time() - t, float('inf'), None, _SENTINEL)) # Re-propagate sentinel
            break

        prof.t_dequeue = time.time() - t
        global_executer.submit(dispatcher_thread, ts, idx, prof, frame)
        print(f"frame {idx} pushed to global executer for processing")

    # ← loop has exited: wait for every frame already submitted
    global_executer.shutdown(wait=True)  
    print("work_allocator finished; all dispatched tasks completed.")
        
def dispatcher_thread(ts, idx, prof, frame):

    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as pool:
        print(f"Dispatcher thread started for frame {idx}. Waiting for frames...")
        
        # Submit tasks and store futures along with their keys
        future_to_key = {
        ...
        total_tasks_for_frame = len(future_to_key)
        ...
        print(f"{prof.idx} : (Absolute times: Enqueued: {prof.t_enqueue:.4f}, Dequeued: {prof.t_dequeue:.4f}, "
              f"Shape End: {prof.t_shape:.4f}, "
              f"Cook End: {prof.t_cook:.4f}, "
              f"Hair End: {prof.t_hair:.4f})")

    print(f"Dispatcher thread exited for frame {idx}")
...
def detect_hair(
    frame_np: np.ndarray,
    model_weight_path: str = "/content/drive/MyDrive/deeplabv3_efficientnetb4_epoch200.pth",
    grid_rows: int = 3,
    grid_cols: int = 3,
    patch_size: int = 1024,
    ...
    def process_patch(i, j):
        nonlocal full_mask
        with lock:  # serialize GPU access
...
def process_single_image(image_input):
    ...
    def remove_bg(image_bgr):
        # Resize using OpenCV (BGR image)
        new_size = (640, 480)  # Note: OpenCV uses (width, height)
        resized_bgr = cv2.resize(image_bgr, new_size, interpolation=cv2.INTER_LANCZOS4)

        # Convert BGR to RGB for rembg/PIL
